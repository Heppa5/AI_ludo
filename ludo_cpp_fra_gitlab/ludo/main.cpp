...
const int number_of_games_per_individual=250;
...
void evaluation_function(game *g,QApplication* a, vector<individual> *pop, int index, ludo_player *p1)
{
    ...
    //population[index].evaluation = population[index].evaluation + population[index].wins*50;
}
...
int main(int argc, char *argv[]){
   ...
   /* Dialog w;
    ...
    while (done==false)
    {
        ...
        select_best_offspring(&population, &children );
        //population=children;
       // std::sort(population.begin(), population.end(), compareBySize);
        generation++;
